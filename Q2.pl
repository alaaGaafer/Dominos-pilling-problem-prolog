...
place_dominos_helper(Board, Positions, Dominos) :-
  ...
  heuristic(Dominos, Dominos2) :-
    heuristic(Dominos, [], 0, MaxNum),
    include(length_equals(MaxNum), Dominos, Dominos2).

heuristic([], _, MaxNum, MaxNum).
heuristic([H|T], _, CurrentMaxNum, MaxNum) :-
    length(H, L),
    (L > CurrentMaxNum ->
        heuristic(T, [H], L, MaxNum)
    ;
        L =:= CurrentMaxNum ->
        heuristic(T, [H|_], CurrentMaxNum, MaxNum)
    ;
        heuristic(T, _, CurrentMaxNum, MaxNum)
    ).
length_equals(X, L) :-
    length(L, X).
...
solve(M, N, Bomb1, Bomb2, Dominos2) :- 
  numlist(1, M, Rows),
  numlist(1, N, Columns),
